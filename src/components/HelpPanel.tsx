...
export function HelpPanel() {
  ...
  return (
    <>
      ...
      <div className="fixed left-1/2 top-1/2 z-50 w-[420px] -translate-x-1/2 -translate-y-1/2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 shadow-2xl">
        ...
        <ul className="max-h-[60vh] overflow-y-auto px-5 py-3">
          {SHORTCUTS.map((s) => (
            <li key={s.label} className="flex items-center justify-between border-b border-gray-50 dark:border-gray-800 py-2 text-sm last:border-0">
              <span className="text-gray-600 dark:text-gray-300">{s.label}</span>
              <kbd className="rounded border border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-700 px-2 py-0.5 font-mono text-xs text-gray-500 dark:text-gray-400">
                {s.keys}
              </kbd>
            </li>
          ))}
